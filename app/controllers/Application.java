...
public class Application extends Controller {
	...
	private static Confluence getConfluence() {
		...
		if (confluence == null) {
			confluence = new Confluence(
					URI.create("http://wiki.java.no/rest/atompub/latest/"));
		}
		return confluence;
	}
	...
	public static void index() {
		...
		ArrayList<Announcement> announcements = Cache.get("announcements",
				ArrayList.class);

		if (announcements == null) {
			try {
				Future<Collection<NewsItem>> forsiden = getConfluence()
						.getNewsFeed(
								"Forsiden",
								new DateTime().minus(Days.days(5)
										.toStandardDuration()));
				Collection<NewsItem> items = forsiden.get();
				announcements = Lists.newArrayList(Collections2.transform(
						items, new Function<NewsItem, Announcement>() {

							@Override
							public Announcement apply(final NewsItem newsItem) {

								return new Announcement(newsItem.getTitle(),
										newsItem.getBody(), newsItem.getUri()
												.toString());
							}
						}));
		...
		List<Event> events = Event.find(
				"published is true and date >= ? order by date asc",
				new DateMidnight().plus(1).toDate()).fetch();
		String randomId = Codec.UUID();
		render(announcements, events, randomId);
	}

	public static void signUpForEvent(final Long eventId,
			final String randomId, final String code,
			@Required @Email final String email, @Required final String name,
			@Required final Integer howMany) {

		validation.equals(code.toLowerCase(), Cache.get(randomId)).message(
				"Feil kode!");
		validation.match(howMany, "[1-3]").message(
				"Feltet må være et siffer mellom 1 og 3");
		if (!validation.hasErrors()) {
			Participant participant = null;
			List<Participant> participantList = Participant.find("email = ?",
					email).fetch();
			...
			if (!event.participants.contains(participant)) {
				...
				event.save();
				String crypto = Crypto.encryptAES(participant.id + "_"
						+ event.id);
	...
	public static void regretSigningUp(final String id) {
		...
		if (strings != null && strings.length == 2) {
			Participant participant = Participant.findById(Long
					.parseLong(strings[0]));
	...
	public static void listOldEvents() {
		...
		List<Event> osloEvents = Event
				.find("published is true and region = ? and date < ? order by date desc",
						Event.Region.OSLO, today).fetch();
		List<Event> trondheimEvents = Event
				.find("published is true and region = ? and date < ? order by date desc",
						Event.Region.TRONDHEIM, today).fetch();
		List<Event> sorlandetEvents = Event
				.find("published is true and region = ? and date < ? order by date desc",
						Event.Region.SORLANDET, today).fetch();
		List<Event> bergenEvents = Event
				.find("published is true and region = ? and date < ? order by date desc",
						Event.Region.BERGEN, today).fetch();
		List<Event> stavangerEvents = Event
				.find("published is true and region = ? and date < ? order by date desc",
						Event.Region.STAVANGER, today).fetch();
		render(osloEvents, trondheimEvents, sorlandetEvents, bergenEvents,
				stavangerEvents);
	}
	...
	public static void confluence(final String name) {
		...
		if (document == null) {
			try {
				Future<Page> pageFuture = getConfluence().getPage("Forsiden",
						name);
	...
	private static String transform(final Page page) {
		...
		try {
			Transformer transformer = tFactory
					.newTransformer(new StreamSource(Application.class
							.getResourceAsStream(FLATPAGE_TRANSFORMATION_RULES)));
			transformer.transform(new StreamSource(new StringReader(xHtml)),
	...
	public static void sendtip(@Valid final Tip tip) {

		Logger.debug("Received new tip %s", tip);
